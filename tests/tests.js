...
check(v.validate(user).match(['superuser', 'megauser']).isValid(), 'matches array');
...
check(v.validate("{'test':'best'}").isJson().isValid() === false, 'isJson');
check(v.validate('{"test":"best"}').isJson().isValid(), 'isJson');
